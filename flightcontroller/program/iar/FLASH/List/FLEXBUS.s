...
//    Locale       =  C
//    List file    =  
//        C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\iar\FLASH\List\FLEXBUS.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   62   ASSERT( Fbx <= 5);    //判断FB通道号
        ...
        CMP      R0,#+6
        BLT.N    ??FlexBus_Init_1
        MOVS     R1,#+62
        LDR.N    R0,??FlexBus_Init_0
        BL       assert_failed
...
//   69   FB->CS[Fbx].CSMR = 0;
??FlexBus_Init_7:
        LDR.N    R1,??FlexBus_Init_0+0x4  ;; 0x4000c000
        MOVS     R2,#+12
        MOVS     R0,R6
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        MULS     R0,R2,R0
...
//   98     PORTD->PCR[1]  = PORT_PCR_MUX(5);       // FB_CS0     
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x8  ;; 0x4004c004
        STR      R0,[R1, #+0]
        B.N      ??FlexBus_Init_11
//   99   }
//  100   else if(Fbx == FB1)
...
//  102     FB->CSPMCR &= ~(FB_CSPMCR_GROUP1_MASK);
        LDR.N    R0,??FlexBus_Init_0+0xC  ;; 0x4000c060
        LDR      R1,[R0, #+0]
...
//  104     PORTD->PCR[0]  = PORT_PCR_MUX(5);       // FB_CS1  
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x10  ;; 0x4004c000
        STR      R0,[R1, #+0]
        B.N      ??FlexBus_Init_11
//  105   }
//  106   else if(Fbx == FB2)
...
//  110     PORTC->PCR[18]  = PORT_PCR_MUX(5);       // FB_CS2
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x14  ;; 0x4004b048
        STR      R0,[R1, #+0]
        B.N      ??FlexBus_Init_11
//  111   }
//  112   else if(Fbx == FB3)
...
//  116     PORTC->PCR[19]  = PORT_PCR_MUX(5);       // FB_CS3
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x18  ;; 0x4004b04c
        STR      R0,[R1, #+0]
        B.N      ??FlexBus_Init_11
//  117   }
//  118   else if(Fbx == FB4)
...
//  122     PORTC->PCR[17]  = PORT_PCR_MUX(5);       // FB_CS4
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x1C  ;; 0x4004b044
        STR      R0,[R1, #+0]
        B.N      ??FlexBus_Init_11
//  123   }
//  124   else if(Fbx == FB5)
...
//  128     PORTC->PCR[16]  = PORT_PCR_MUX(5);       // FB_CS5
        MOV      R0,#+1280
        LDR.N    R1,??FlexBus_Init_0+0x20  ;; 0x4004b040
        STR      R0,[R1, #+0]
//  129   }
//  130   
//  131   //使能总线地址、数据引脚复用功能
...
??FlexBus_Init_24:
        MOV      R0,#+1280
        LDR.N    R2,??FlexBus_Init_0+0x28  ;; 0x4004a04c
        STR      R0,[R2, #+0]
//  171   PORTC->PCR[11] = PORT_PCR_MUX(5);          // fb_rw_b  
        LDR.N    R2,??FlexBus_Init_0+0x2C  ;; 0x4004b02c
        STR      R0,[R2, #+0]
//  172               
//  173   return 1;
        MOVS     R0,#+1
