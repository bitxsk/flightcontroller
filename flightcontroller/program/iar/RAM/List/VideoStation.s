...
//    Command line =  
//        -f C:\Users\XSK199~1\AppData\Local\Temp\EW37A6.tmp
//        (C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\app\Sensor\VideoStation.cpp
...
//    Locale       =  C
//    List file    =  
//        C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\iar\RAM\List\VideoStation.s
//
///////////////////////////////////////////////////////////////////////////////
...
_ZN13CVideoStationC1Ev:
        ...
        VMOV.F32 S1,#20.0
        MOVS     R1,#+100
        VLDR.W   S0,??CVideoStation_0  ;; 0x3e19999a
        ADDW     R0,R4,#+2036
...
//   14   videoStation->positionFilterX.setParameter(100, 100);
        MOVS     R1,#+100
        VLDR.W   S0,??DataTable0  ;; 0x42c80000
...
//   15   videoStation->positionFilterX.maxValue = 240;
        LDR.W    R0,??DataTable4_6
        LDR      R0,[R0, #+0]
        LDR.W    R1,??DataTable4_7  ;; 0x43700000
        STR      R1,[R0, #+2160]
...
//   20   videoStation->positionFilterY.maxValue = 320;
        LDR.W    R0,??DataTable4_6
        LDR      R0,[R0, #+0]
        LDR.W    R1,??DataTable4_8  ;; 0x43a00000
        STR      R1,[R0, #+2216]
...
??DataTable0:
        ...
        DATA
// __absolute unsigned char VideoStation_Init()::static guard for videoStation_0
...
//   29   int address0 = (DMA0->TCD[DMA_CHn_Rx].DADDR) - (uint32)(&uartData[0]);
        LDR.W    R0,??DataTable4_9  ;; 0x40009000
        LDRB     R1,[R5, #+0]
        LSLS     R1,R1,#+5
        ADD      R0,R0,R1
        LDR      R0,[R0, #+16]
...
//   63     }
//   64     
//   65 //    char checkSum = 0; //注意每个包的checkSum都要清零
...
??updateUart_17:
        ...
        BEQ.N    ??updateUart_8
//  103     {
//  104       float distanceCmaera = 1.54;//Navigation->DesiredDistanceToScreen - EKF_AHRS->fusion.position.x + 0.1; //0.1是相机离超声波的距离
        VLDR.W   S16,??DataTable3  ;; 0x3fc51eb8
//  105       
//  106       packageTop += 1;
        ADDS     R10,R10,#+1
//  107       positionX_raw = (int16)(uartData[packageTop + 1] << 8 | uartData[packageTop]); //x坐标 单位像素
        ADD      R0,R5,R10
        LDRB     R0,[R0, #+3]
        ADD      R1,R5,R10
        LDRB     R1,[R1, #+2]
...
//  109       positionX_A = (positionX_F - 120) * distanceCmaera / 192;
        LDR      R0,[R5, #+2260]
        VMOV     S0,R0
        VLDR.W   S1,??DataTable4  ;; 0xc2f00000
        VADD.F32 S0,S0,S1
        VMUL.F32 S0,S0,S16
        VLDR.W   S1,??DataTable4_1  ;; 0x43400000
        VDIV.F32 S0,S0,S1
        ADDW     R0,R5,#+2268
        VSTR     S0,[R0, #0]
//  110       
//  111       packageTop += 2;
        ADDS     R10,R10,#+2
//  112       positionY_raw = (int16)(uartData[packageTop + 1] << 8 | uartData[packageTop]); //y坐标
        ADD      R0,R5,R10
        LDRB     R0,[R0, #+3]
        ADD      R1,R5,R10
        LDRB     R1,[R1, #+2]
...
//  127   moveLength = (DMA0->TCD[DMA_CHn_Rx].DADDR - (uint32)&uartData[0]) - i0;
        LDR.N    R0,??DataTable4_9  ;; 0x40009000
        LDRB     R1,[R5, #+0]
        LSLS     R1,R1,#+5
        ADD      R0,R0,R1
        LDR      R0,[R0, #+16]
...
??updateUart_20:
        ADDS     R0,R5,#+2
        ADDS     R0,R10,R0
        LDR.N    R1,??DataTable4_9  ;; 0x40009000
        LDRB     R2,[R5, #+0]
        LSLS     R2,R2,#+5
        ADD      R1,R1,R2
        STR      R0,[R1, #+16]
...
//  147   velFilter_x.accumulate(IMU_acc_E.x * 0.002);
        ...
        LDR.N    R3,??DataTable4_13  ;; 0x3f60624d
        BL       __aeabi_dmul
        BL       __aeabi_d2f
        VMOV     S0,R0
        ADDW     R0,R4,#+2036
...
//  159     int distance = (int)((Navigation->DesiredDistanceToScreen - UltrasoundUART->position.x) * 1000);
        ...
        VLDR     S1,[R0, #+168]
        VSUB.F32 S0,S1,S0
        VLDR.W   S1,??DataTable4_3  ;; 0x447a0000
        VMUL.F32 S0,S0,S1
        VCVT.S32.F32 S16,S0
...
_ZN13CVideoStation3TX_17startDMA_TransmitEv:
        ...
        LDR      R1,[R1, #+0]
        ADDW     R1,R1,#+2548
        LDR.N    R2,??DataTable4_17  ;; 0x400090e0
        STR      R1,[R2, #+0]
//  174   
//  175   //设置主循环计数器 current major loop count
//  176   DMA0->TCD[DMA_CH7].CITER_ELINKNO = DMA_CITER_ELINKNO_CITER(videoStation->TX.index);
        ...
        LSRS     R1,R1,#+17
        LDR.N    R2,??DataTable4_18  ;; 0x400090f6
        STRH     R1,[R2, #+0]
//  177   
//  178   //起始循环计数器，当主循环计数器为零的时候，将装载起始循环计数器的值
//  179   DMA0->TCD[DMA_CH7].BITER_ELINKNO = DMA_CITER_ELINKNO_CITER(videoStation->TX.index);
        ...
        LSLS     R1,R1,#+17       ;; ZeroExtS R1,R1,#+17,#+17
        LSRS     R1,R1,#+17
        LDR.N    R2,??DataTable4_19  ;; 0x400090fe
        STRH     R1,[R2, #+0]
//  180     
//  181   DMA_EnableReq(DMA_CH7);
        LDR.N    R1,??DataTable4_20  ;; 0x4000800c
        LDR      R1,[R1, #+0]
        ORRS     R1,R1,#0x80
        LDR.N    R2,??DataTable4_20  ;; 0x4000800c
        STR      R1,[R2, #+0]
//  182 }
        BX       LR               ;; return
...
// 
// 1 270 bytes of CODE  memory (+ 248 bytes shared)
