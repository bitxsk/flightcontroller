...
//        decode.cpp
//    Command line =  
//        -f C:\Users\XSK199~1\AppData\Local\Temp\EW33AB.tmp
//        ("C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\app\Sensor\UWB
...
//    Locale       =  C
//    List file    =  
//        C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\iar\RAM\List\UWB
//        decode.s
//
///////////////////////////////////////////////////////////////////////////////
...
//    9   UWB->velFilter_x.setParameter(0.15, 100, 20);
        VMOV.F32 S1,#20.0
        MOVS     R1,#+100
        VLDR.W   S0,??DataTable2  ;; 0x3e19999a
...
??UWB_Init_1:
        ...
        DATA
// __absolute unsigned char UWB_Init()::static guard for UWB_0
...
//   30   int address0 = (DMA0->TCD[DMA_CH4].DADDR) - (uint32)(&Data[0]);
        LDR.W    R0,??DataTable4_4  ;; 0x40009090
        LDR      R7,[R0, #+0]
        SUBS     R7,R7,R5
//   31   int moveLength = 0;
        MOVS     R8,#+0
...
//   53       break;
//   54     }
//   55     char checkSum = 0; //注意每个包的checkSum都要清零
??update_6:
        MOVS     R10,#+0
...
//   86   moveLength = (DMA0->TCD[DMA_CH4].DADDR - (uint32)&Data[0]) - i;
        LDR.W    R1,??DataTable4_4  ;; 0x40009090
        LDR      R1,[R1, #+0]
        SUBS     R1,R1,R5
        SUBS     R1,R1,R9
...
??update_11:
        ADDS     R2,R1,R5
        LDR.W    R3,??DataTable4_4  ;; 0x40009090
        STR      R2,[R3, #+0]
...
//   97     DMA_LoadDstAddr(DMA_CH4, (uint32)&Data[0]);
        LDR.W    R2,??DataTable4_4  ;; 0x40009090
        STR      R5,[R2, #+0]
//   98   }
//   99 }
??update_14:
...
//  126   double distanceZsqr = (1.4 - EKF_AHRS->fusion.position.z) * (1.4 - EKF_AHRS->fusion.position.z);
        ...
        MOVS     R3,R1
        MOVS     R0,#+1717986918
        LDR.N    R1,??DataTable4_6  ;; 0x3ff66666
        BL       __aeabi_dsub
        MOVS     R4,R0
        MOVS     R5,R1
        VMOV     R0,S17
        ...
        BL       __aeabi_dmul
        VMOV     D8,R0,R1
//  127   double distanceXsqr = (EKF_AHRS->fusion.position.x - Navigation->DesiredDistanceToScreen / 2.0f)  * //距超宽带X方向距离
//  128                           (EKF_AHRS->fusion.position.x - Navigation->DesiredDistanceToScreen / 2.0f); 
        ...
        BL       __aeabi_f2d
        VMOV     D10,R0,R1
//  129   arm_sqrt_f32(Py * Py - distanceZsqr - distanceXsqr, &norm); //计算水平距离
        MOV      R0,SP
        MOVS     R4,R0
        VMUL.F32 S0,S18,S18
        VMOV     R0,S0
...
//  130   positionF.y = 7.4 - norm; //7.4是自定义的原点位置
        LDR      R0,[SP, #+0]
        BL       __aeabi_f2d
        MOVS     R2,R0
        MOVS     R3,R1
        LDR.N    R0,??DataTable4_8  ;; 0x9999999a
        LDR.N    R1,??DataTable4_9  ;; 0x401d9999
        BL       __aeabi_dsub
        BL       __aeabi_d2f
        VMOV     S0,R0
        ADDS     R0,R6,#+1656
        BL       _ZN17CoordinateElementaSEf
//  131   positionF.x = positionFilter_x.newData(positionRaw.y, t); //x y轴换一下
        ADDS     R0,R6,#+1320
        BL       _ZN17CoordinateElementcvfEv
        LDR.N    R0,??DataTable4_3
        LDR      R1,[R0, #+0]
...
_ZN4CUWB3TX_17startDMA_TransmitEv:
        ...
        LDR      R1,[R1, #+0]
        ADDW     R1,R1,#+1820
        LDR.N    R2,??DataTable4_10  ;; 0x400090a0
        STR      R1,[R2, #+0]
//  139   
//  140   //设置主循环计数器 current major loop count
//  141   DMA0->TCD[DMA_CH5].CITER_ELINKNO = DMA_CITER_ELINKNO_CITER(UWB->TX.index);
        ...
        LSRS     R1,R1,#+17
        LDR.N    R2,??DataTable4_11  ;; 0x400090b6
        STRH     R1,[R2, #+0]
//  142   
//  143   //起始循环计数器，当主循环计数器为零的时候，将装载起始循环计数器的值
...
//  146   DMA_EnableReq(DMA_CH5);
        ...
        LDR      R1,[R1, #+0]
        ORRS     R1,R1,#0x20
        LDR.N    R2,??DataTable4_13  ;; 0x4000800c
        STR      R1,[R2, #+0]
//  147 }
        BX       LR               ;; return
...
// 
// 1 530 bytes of CODE  memory (+ 200 bytes shared)
