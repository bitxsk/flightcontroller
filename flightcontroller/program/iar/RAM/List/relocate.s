...
//    Locale       =  C
//    List file    =  
//        C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\iar\RAM\List\relocate.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   65       __VECTOR_RAM[n] = __VECTOR_TABLE[n];
        LDR.N    R1,??common_relocate_2
        LDR      R1,[R1, R0, LSL #+2]
        LDR.N    R2,??common_relocate_2+0x4
        STR      R1,[R2, R0, LSL #+2]
        ADDS     R0,R0,#+1
        B.N      ??common_relocate_0
//   66   }
//   67   /* 将新的中断向量表指针赋给VTOR寄存器 */
//   68   write_vtor((uint32)__VECTOR_RAM);
??common_relocate_1:
        LDR.N    R1,??common_relocate_2+0x4
        LDR.N    R2,??common_relocate_2+0x8  ;; 0xe000ed08
        STR      R1,[R2, #+0]
//   69   
//   70   /* 获得.data段的地址(已初始化的数据段) */
//   71   #if defined(__IAR_SYSTEMS_ICC__)
//   72     data_ram = (uint8 *)__section_begin(".data");
        LDR.N    R3,??common_relocate_2+0xC
//   73     data_rom = (uint8 *)__section_begin(".data_init");
        LDR.N    R2,??common_relocate_2+0x10
//   74     data_rom_end = (uint8 *)__section_end(".data_init");
        LDR.N    R1,??common_relocate_2+0x14
...
??common_relocate_4:
        LDR.N    R5,??common_relocate_2+0x18
//   92   bss_end = (uint8 *)__section_end(".bss");  
        LDR.N    R4,??common_relocate_2+0x1C
//   93   #else
