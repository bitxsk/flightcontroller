...
//    Command line =  
//        -f C:\Users\XSK199~1\AppData\Local\Temp\EW126.tmp
//        (C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\lib\LPLD\HW\MCG.c
...
//    Locale       =  C
//    List file    =  
//        C:\Users\xsk19940910\Desktop\flightController-1\flightController-1\program\iar\RAM\List\MCG.s
//
///////////////////////////////////////////////////////////////////////////////
...
??PLL_Setup_15:
        LDR.W    R12,??PLL_Setup_16  ;; 0x4007d002
        LDRB     R12,[R12, #+0]
        LSLS     R12,R12,#+28
        BPL.N    ??PLL_Setup_17
//  360         PMC->REGSC |= PMC_REGSC_ACKISO_MASK;
        ...
        LDRB     R12,[R12, #+0]
        ORRS     R12,R12,#0x8
        LDR.W    LR,??PLL_Setup_16  ;; 0x4007d002
        STRB     R12,[LR, #+0]
...
??PLL_Setup_17:
        ...
        ANDS     LR,LR,#0xF0000
        ORRS     R12,LR,R12
        LDR.W    LR,??PLL_Setup_16+0x4  ;; 0x40048044
        STR      R12,[LR, #+0]
...
//  373   MCG->C2 = MCG_C2_RANGE0(1);
        MOVS     R12,#+16
        LDR.W    LR,??PLL_Setup_16+0x8  ;; 0x40064001
        STRB     R12,[LR, #+0]
...
//  376   MCG->C1 = MCG_C1_CLKS(2) | MCG_C1_FRDIV(5);
        MOVS     R12,#+168
        LDR.W    LR,??PLL_Setup_16+0xC  ;; 0x40064000
        STRB     R12,[LR, #+0]
//  377   while (MCG->S & MCG_S_IREFST_MASK){}; // 等待参考时钟清零
??PLL_Setup_18:
        LDR.W    R12,??PLL_Setup_16+0x10  ;; 0x40064006
        LDRB     R12,[R12, #+0]
        LSLS     R12,R12,#+27
        BMI.N    ??PLL_Setup_18
...
//  383   MCG->C6 |= MCG_C6_CME0_MASK;
        LDR.W    R12,??PLL_Setup_16+0x14  ;; 0x40064005
        LDRB     R12,[R12, #+0]
        ORRS     R12,R12,#0x20
        LDR.W    LR,??PLL_Setup_16+0x14  ;; 0x40064005
        STRB     R12,[LR, #+0]
//  384   MCG->C5 = MCG_C5_PRDIV0(prdiv); // 设置 PLL 匹配晶振的参考分频数
        ANDS     R12,R1,#0x7
        LDR.W    LR,??PLL_Setup_16+0x18  ;; 0x40064004
        STRB     R12,[LR, #+0]
...
//  390   MCG->C1 &= ~MCG_C1_CLKS_MASK;
        LDR.W    R12,??PLL_Setup_16+0xC  ;; 0x40064000
        LDRB     R12,[R12, #+0]
        ANDS     R12,R12,#0x3F
        LDR.W    LR,??PLL_Setup_16+0xC  ;; 0x40064000
        STRB     R12,[LR, #+0]
...
//  442   temp_reg = FMC->PFAPR; // 备份 FMC_PFAPR 寄存器
        LDR.N    R4,??Set_SYS_DIV_0  ;; 0x4001f000
        LDR      R4,[R4, #+0]
//  443   
...
//  447              | FMC_PFAPR_M1PFD_MASK | FMC_PFAPR_M0PFD_MASK;
        LDR.N    R5,??Set_SYS_DIV_0  ;; 0x4001f000
        LDR      R5,[R5, #+0]
        ORRS     R5,R5,#0xFF0000
...
//  451               | SIM_CLKDIV1_OUTDIV3(outdiv3) | SIM_CLKDIV1_OUTDIV4(outdiv4);
        ...
        ANDS     R6,R6,#0xF0000
        ORRS     R5,R6,R5
        LDR.N    R6,??Set_SYS_DIV_0+0x4  ;; 0x40048044
        STR      R5,[R6, #+0]
//  452 
//  453   // 延时一小段时间等待改变
