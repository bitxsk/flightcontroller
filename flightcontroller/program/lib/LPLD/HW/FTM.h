...
uint8 FTM_Init(FTM_InitTypeDef);
//FTM模块通用反初始化
uint8 FTM_Deinit(FTM_InitTypeDef);
//FTM模块PWM模式输出使能，配置输出通道、占空比、指定对应的引脚、对齐方式
uint8 FTM_PWM_Enable(FTM_Type *, FtmChnEnum_Type, uint32, PortPinsEnum_Type, uint8);
//变更FTM模块PWM模式指定通道的输出占空比
uint8 FTM_PWM_ChangeDuty(FTM_Type *, FtmChnEnum_Type, uint32);
//FTM模块输入捕获模式输出使能，配置输入通道、指定对应的引脚、捕获边缘
uint8 FTM_IC_Enable(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type, uint8);
//禁用FTM模块指定通道的输出、输入
uint8 FTM_DisableChn(FTM_Type *, FtmChnEnum_Type);
//FTM模块正交解码模式输入使能，配置输入通道PHA和PHB
uint8 FTM_QD_Enable(FTM_Type *, PortPinsEnum_Type, PortPinsEnum_Type);
//FTM模块正交解码模式输入禁用
uint8 FTM_QD_Disable(FTM_Type *);

//判断FTMx是否产生计数溢出中断标志
boolean FTM_IsTOF(FTM_Type *);
//清除FTMx计数器溢出中断标志
void FTM_ClearTOF(FTM_Type *);

//判断通道n是否产生中断标志
inline boolean FTM_IsCHnF(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
  return (boolean)((ftmx->CONTROLS[chn].CnSC & FTM_CnSC_CHF_MASK)>>FTM_CnSC_CHF_SHIFT);
}

//清除通道n中断标志
inline uint16 FTM_GetChVal(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
  return ftmx->CONTROLS[chn].CnV;    //获取相应通道捕获到的FTM计数器值
}

inline void FTM_ClearCHnF(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
  ftmx->CONTROLS[chn].CnSC = ftmx->CONTROLS[chn].CnSC & ~FTM_CnSC_CHF_MASK;
}

//获取FTMx通道n捕获的FTMx计数值
uint16 FTM_GetChVal(FTM_Type *, FtmChnEnum_Type);
//获取FTMx时钟分频系数
uint8 FTM_GetClkDiv(FTM_Type *);
//获取FTMx计数器值
uint16 FTM_GetCounter(FTM_Type *);

//清空FTMx计数器
inline void FTM_ClearCounter(FTM_Type *ftmx)
{
  ftmx->CNT = 0;        //清空FTMx计数器值
}

//使能FTMx中断
uint8 FTM_EnableIrq(FTM_InitTypeDef);
//禁用FTMx中断
uint8 FTM_DisableIrq(FTM_InitTypeDef);
}
#endif /* __FTM_H__ */