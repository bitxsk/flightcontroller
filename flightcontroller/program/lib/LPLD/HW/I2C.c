 ...
 *    1--配置成功
 */
uint8 I2C_Init(I2C_InitTypeDef i2c_init_structure)
{
 ...
 *
 */  
uint8 I2C_Deinit(I2C_InitTypeDef i2c_init_structure)
{
  I2C_Type *i2cx = i2c_init_structure.I2C_I2Cx;
 ...
 *    无
 */
void I2C_WaitAck(I2C_Type *i2cx, uint8 is_wait)
{
  int time_out;
  if(is_wait == I2C_ACK_ON)
  {
    ...
    {
      if(time_out>80000) //如果等待超时，强行退出 60000
      {
        I2C_Timeout_myCont++;
 ...
 *
 */
void I2C_WriteByte(I2C_Type *i2cx, uint8 data8)
{
  i2cx->D = data8; 
}
...
uint8 I2C_ReadByte(I2C_Type *i2cx)
{
  uint8 temp;
  temp = i2cx->D; 
  return temp;
 ...
 *    无
 */
void I2C_SetMasterWR(I2C_Type *i2cx, uint8 mode)
{
  if(mode==I2C_MRSW) 
    i2cx->C1 &= (~I2C_C1_TX_MASK);
  else
 ...
 *    无
 */
void I2C_StartTrans(I2C_Type *i2cx, uint8 addr, uint8 mode)
{
  //I2C产生start信号
  I2C_Start(i2cx);
  //将从机地址和主机读写位合成一个字节写入
