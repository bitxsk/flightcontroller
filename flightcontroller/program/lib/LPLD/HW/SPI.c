 ...
 *    1--配置成功
 */
uint8 SPI_Init(SPI_InitTypeDef spi_init_structure)
{
  ...
  spix->CTAR[0] |=SPI_CTAR_ASC(1);
    ...
    //配置SPI CTAR寄存器，设置SPI的总线时序
  spix->CTAR[1] = 0 & (~SPI_CTAR_LSBFE_MASK)
                      |SPI_CTAR_DBR_MASK  
                      |SPI_CTAR_PBR(0)           
                      |SPI_CTAR_FMSZ(7);        
  ...
  //设置SPI总线频率
  //SCK总线频率 = g_bus_clock/ SCK_DIV_x                                             
  spix->CTAR[1] |=SPI_CTAR_BR(sck_div);

  //tCSC = (1/g_bus_clock) x PCSSCK x CSSCK
  //tCSC = 1/50,000,000 x PCSSCK x CSSCK
  spix->CTAR[1] |=SPI_CTAR_PCSSCK(1); 
  spix->CTAR[1] |=SPI_CTAR_CSSCK(1);  
  
  //tDT = (1/g_bus_clock) x PDT x DT
  spix->CTAR[1] |=SPI_CTAR_DT(1);
  spix->CTAR[1] |=SPI_CTAR_PDT(1);
  ...
  //Config the Delay of the last edge of SCK and the negation of PCS
  //tASC = (1/g_bus_clock) x PASC x ASC
  spix->CTAR[1] |=SPI_CTAR_PASC(1);
  spix->CTAR[1] |=SPI_CTAR_ASC(1);
 ...
 *    1--配置成功
 */
uint8 SPI_Deinit(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  spix->MCR |= SPI_MCR_HALT_MASK; //
  if(spix == SPI0)
 ...
 *    1--配置成功
 */
uint8 SPI_EnableIrq(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  if(spix == SPI0)
  {
 ...
 *    1--配置成功
 */
uint8 SPI_DisableIrq(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  if(spix == SPI0)
  {
 ...
 *    读取从机8位的数据
 */
uint8 SPI_Master_WriteRead(SPI_Type *spix,uint8 data,uint8 pcsx,uint8 pcs_state)
{
  uint8 temp;
  
  spix->PUSHR  = (((uint32_t)(((uint32_t)(pcs_state))<<SPI_PUSHR_CONT_SHIFT))&SPI_PUSHR_CONT_MASK)
               |SPI_PUSHR_CTAS(SPI1_transferAttributeRegisters)
               |SPI_PUSHR_PCS(pcsx)
               |data;                 
 ...
 *    读取从机8位的数据
 */
uint8 SPI_Master_Read(SPI_Type *spix)
{
  uint8 temp;
 ...
 *      |__SPI_PCS_INACTIVE -片选无效,PCS信号变为高电平
 */
void SPI_Master_Write(SPI_Type *spix,uint8 data,uint8 pcsx,uint8 pcs_state)
{  
...
int SPI1_PUSHR;
int SPI1_transferAttributeRegisters;
void SPI1_transferAttributeRegistersSwitch(int num)
{
  SPI1_transferAttributeRegisters = num;
