...
//PDB_AdcTriggerCfg函数cfg形参取值定义：使能ADCx通道CHn、是否延时输出、是否BB模式
...
//PDB_DacTriggerCfg函数cfg形参取值定义：使能DACx间隔触发、是否外部触发
...
//使能PDBx中断
#define PDB_EnableIrq()    enable_irq(PDB0_IRQn)
//禁用PDBx中断
#define PDB_DisableIrq()   disable_irq(PDB0_IRQn)
//软件触发PDBx计数器开始工作
#define PDB_SoftwareTrigger()      (PDB0->SC|=PDB_SC_SWTRIG_MASK)

//PDBx模块通用初始化函数，初始化PDBx计数器周期、工作模式、触发源、中断等配置
uint8 PDB_Init(PDB_InitTypeDef);
//PDBx模块反初始化函数，禁用PDBx所有功能
void PDB_Deinit(void);
//PDBx触发ADCx模块配置函数
uint8 PDB_AdcTriggerCfg(ADC_Type *, uint32, uint16);
//PDBx触发DACx模块配置函数
uint8 PDB_DacTriggerCfg(DAC_Type *, uint32, uint16);
