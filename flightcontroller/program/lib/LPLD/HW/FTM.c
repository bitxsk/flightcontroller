...
static uint8 FTM_PWM_Init(FTM_InitTypeDef);
static uint8 FTM_IC_Init(FTM_InitTypeDef);
static uint8 FTM_QD_Init(FTM_InitTypeDef);
static uint8 FTM_DEC_Init(FTM_InitTypeDef);
static uint8 FTM_PinInit(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type);
static uint8 FTM_PinDeinit(FTM_Type *, FtmChnEnum_Type);
 ...
 *    1--配置成功
 */
uint8 FTM_Init(FTM_InitTypeDef ftm_init_structure)
{
  uint8 result, i;
  //参数检查
 ...
 *    1--配置成功
 */
uint8 FTM_Deinit(FTM_InitTypeDef ftm_init_structure)
{
  if(ftm_init_structure.FTM_Ftmx == FTM0)
  {
    SIM->SCGC6 &= ~SIM_SCGC6_FTM0_MASK;
  ...
  return FTM_DisableIrq(ftm_init_structure);
}
 ...
 *    1--配置成功
 */
uint8 FTM_PWM_Enable(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32 duty, PortPinsEnum_Type pin, uint8 align)
{
  uint32 cv;
  vuint32 mod;
 ...
 *    1--配置成功
 */
uint8 FTM_PWM_ChangeDuty(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32 duty)
{
  uint32 cv;
  vuint32 mod;
  ...
  //参数检查
  ASSERT( duty <= 10000 );                  //判断占空比
  //if(duty < 0) duty = 0;      //Warning[Pe186]: pointless comparison of unsigned integer with zero.
 ...
 *    1--配置成功
 */
uint8 FTM_DisableChn(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
  FTM_PinDeinit(ftmx, chn);
 ...
 *    1--配置成功
 */
uint8 FTM_IC_Enable(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin, uint8 capture_edge)
{
  if(!FTM_PinInit(ftmx, chn, pin))
    return 0;
 ...
 *
 */
__INLINE boolean FTM_IsTOF(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE void FTM_ClearTOF(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE uint8 FTM_GetClkDiv(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE uint16 FTM_GetCounter(FTM_Type *ftmx)
{
 ...
 *
 */
uint8 FTM_EnableIrq(FTM_InitTypeDef ftm_init_structure)
{
  uint8 i;
 ...
 *
 */
uint8 FTM_DisableIrq(FTM_InitTypeDef ftm_init_structure)
{
  uint8 i;
  FTM_Type *ftmx = ftm_init_structure.FTM_Ftmx;
 ...
 *    1--配置成功
 */
uint8 FTM_QD_Enable(FTM_Type *ftmx, PortPinsEnum_Type pha, PortPinsEnum_Type phb)
{
  if(!FTM_PinInit(ftmx, FTM_PhA, pha))
    return 0;
  if(!FTM_PinInit(ftmx, FTM_PhB, phb))
    return 0;
 ...
 *    1--配置成功
 */
uint8 FTM_QD_Disable(FTM_Type *ftmx)
{
  if(!FTM_PinDeinit(ftmx, FTM_PhA))
    return 0;
  if(!FTM_PinDeinit(ftmx, FTM_PhB))
    return 0;
 ...
 * FTM模块PWM模式初始化，内部调用
 */
static uint8 FTM_PWM_Init(FTM_InitTypeDef ftm_init_structure)
{
  uint32 bus_clk_hz;
  uint32 mod, mod2;
  uint8 ps;
 ...
 * FTM模块输入捕获模式初始化，内部调用
 */
static uint8 FTM_IC_Init(FTM_InitTypeDef ftm_init_structure)
{
  uint8 i;
 ...
 */
static uint8 FTM_PinInit(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin)
{ 
  //根据ftmx使能相应pin的ftm功能
  if(ftmx == FTM0)
  {
 ...
 */
static uint8 FTM_PinDeinit(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 * FTM模块正交解码模式初始化，内部调用
 */
static uint8 FTM_QD_Init(FTM_InitTypeDef ftm_init_structure)
{
...
static uint8 FTM_DEC_Init(FTM_InitTypeDef ftm_init_structure)
{
