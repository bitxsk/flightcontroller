 ...
 *    1--配置成功
 */
uint8 ADC_Init(ADC_InitTypeDef adc_init_structure)
{
  uint8 i;
 ...
 *    1--配置成功
 */
uint8 ADC_Deinit(ADC_InitTypeDef adc_init_structure)
{
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
  uint8 hwtrg = adc_init_structure.ADC_HwTrgCfg;
 ...
 *
 */
uint16 ADC_Get(ADC_Type *adcx, AdcChnEnum_Type chn)
{
 ...
 *
 */
void ADC_EnableConversion(ADC_Type *adcx, AdcChnEnum_Type chn, uint8 ab, boolean irq)
{
  //参数检查
 ...
 *
 */
uint16 ADC_GetResult(ADC_Type *adcx, uint8 ab)
{
  //参数检查
  ASSERT( ab<=1 );  //判断AB控制寄存器
  return adcx->R[ab];
 ...
 *
 */
uint8 ADC_GetSC1nCOCO(ADC_Type *adcx)
{
 ...
 *
 */
uint8 ADC_EnableIrq(ADC_InitTypeDef adc_init_structure)
{
  uint8 i;
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
 ...
 *
 */
uint8 ADC_DisableIrq(ADC_InitTypeDef adc_init_structure)
{
  uint8 i;
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
 ...
 *
 */
uint8 ADC_Chn_Enable(ADC_Type *adcx, AdcChnEnum_Type chn)
{
  //判断复用引脚是a或b
 ...
 *    1--配置成功
 */
static uint8 ADC_Cal(ADC_Type *adcx)
{
  uint16 cal_var;
