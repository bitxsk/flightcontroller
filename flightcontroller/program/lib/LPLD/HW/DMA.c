 ...
 *    1--配置成功
 */
uint8 DMA_Init(DMA_InitTypeDef dma_init_struct)
{
...
char DMA_changeLength(uint8 chx, int length)
{
  //主的计数次数（major iteration count）达到后，重新调整源地址
  DMA0->TCD[chx].SLAST = DMA_SLAST_SLAST(-length);
  //设置主循环计数器 current major loop count
  DMA0->TCD[chx].CITER_ELINKNO = DMA_CITER_ELINKNO_CITER(length);
  //起始循环计数器，当主循环计数器为零的时候，将装载起始循环计数器的值
  DMA0->TCD[chx].BITER_ELINKNO = DMA_CITER_ELINKNO_CITER(length);
  return 1;
}
 ...
 *    1--成功
 */
uint8 DMA_EnableIrq(DMA_InitTypeDef dma_init_struct)
{
#if defined(CPU_MK60DZ10) || defined(CPU_MK60D10) 
 ...
 *    1--成功
 */
uint8 DMA_DisableIrq(DMA_InitTypeDef dma_init_struct)
{
#if defined(CPU_MK60DZ10) || defined(CPU_MK60D10) 
