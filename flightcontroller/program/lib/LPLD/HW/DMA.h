...
//使能通道x的DMA请求
#define DMA_EnableReq(CHX)  (DMA0->ERQ|=(1<<CHX))
//禁用通道x的DMA请求
#define DMA_DisableReq(CHX) (DMA0->ERQ&=~(1<<CHX))
//加载源地址
#define DMA_LoadSrcAddr(CHX, ADDR)   (DMA0->TCD[CHX].SADDR=DMA_SADDR_SADDR(ADDR))
//加载目的地址
#define DMA_LoadDstAddr(CHX, ADDR)   (DMA0->TCD[CHX].DADDR=DMA_DADDR_DADDR(ADDR))

inline bool DMA_isOccupying(uint8 chx)
{
  return DMA0->ERQ & (1 << chx);
}

//初始化eDMA模块
uint8 DMA_Init(DMA_InitTypeDef);
//使能eDMA中断
uint8 DMA_EnableIrq(DMA_InitTypeDef);
//禁用eDMA中断
uint8 DMA_DisableIrq(DMA_InitTypeDef);

char DMA_changeLength(uint8, int);

//DMA服务请求软件开始
/*
 * DMA_SoftwareStartService
 * DMA服务请求软件开始
 * 
 * 参数:
 ...
 *    无
 */
inline void DMA_SoftwareStartService(DMA_InitTypeDef dma_init_struct)
{
