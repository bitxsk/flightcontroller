...
//ENET模块初始化
void ENET_Init(ENET_InitTypeDef);
//以太帧发送函数
void ENET_MacSend(uint8*, uint16);
//以太帧接收函数
uint8 ENET_MacRecv(uint8*, uint16*);
//生成给定的MAC地址的哈希表
uint8 ENET_HashAddress(const uint8*);
//设置MAC物理地址
void ENET_SetAddress(const uint8*);
//设置ENET模块的MII接口时钟
void ENET_MiiInit(uint32);
//MII接口写
uint8 ENET_MiiWrite(uint16 , uint16, uint16);
//MII接口读
uint8 ENET_MiiRead(uint16, uint16, uint16*);
