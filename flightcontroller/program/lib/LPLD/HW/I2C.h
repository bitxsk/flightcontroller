...
uint8 I2C_Init(I2C_InitTypeDef);
//I2Cx反初始化函数，关闭I2Cx中断，禁用I2Cx
uint8 I2C_Deinit(I2C_InitTypeDef);
//I2Cx中断使能函数
...
//I2Cx等待ACK信号函数
void I2C_WaitAck(I2C_Type *, uint8);
//I2Cx再次产生开始信号函数
void I2C_ReStart(I2C_Type *);
//I2Cx发送一个字节函数
void I2C_WriteByte(I2C_Type *, uint8);
//I2Cx读取一个字节函数
uint8 I2C_ReadByte(I2C_Type *);
//I2Cx主机读写模式配置函数
void I2C_SetMasterWR(I2C_Type *, uint8);
//I2C开始传输函数，需要设置外围设备地址和读写模式
void I2C_StartTrans(I2C_Type *, uint8, uint8);



/*
uint8  I2C_WriteAddr(I2C_Type *, uint8 SlaveID, uint8 Addr, uint8 Data);      //读取地址里的内容
uint8  I2C_ReadAddr(I2C_Type *, uint8 SlaveID, uint8 Addr);                //往地址里写入内容
char i2c_write(uint8 addr, uint8 reg, uint8 len, uint8 * data);
uint8 i2cWriteBuffer(uint8 addr, uint8 reg, uint8 len, uint8 * data);
uint8 i2cRead(uint8 addr, uint8 reg, uint8 len, uint8 *buf);
char i2c_read(uint8 addr, uint8 reg, uint8 len, uint8 *buf);//DMP使用的I2C
void  I2C_StartTransmission (I2C_Type *, uint8 SlaveID, uint8);       //启动传输
*/
