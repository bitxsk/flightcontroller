...
//清除扫描结束标志
#define TSI_ClearEndScanFlag()   (TSI0->GENCS|=TSI_GENCS_EOSF_MASK)
//清除范围溢出标志
#define TSI_ClearOutRangeFlag()  (TSI0->GENCS|=TSI_GENCS_OUTRGF_MASK)
//清除通道CHX的扫描接触标志，CHX取值TSI_Ch0~TSI_Ch15
#define TSI_ClearEndScanFlagChx(CHX)   (TSI0->STATUS|=CHX)
//清除通道CHX的错误标志，CHX取值TSI_Ch0~TSI_Ch15
#define TIS_ClrarErrorFlagChx(CHX)     (TSI0->STATUS|=(CHX<<16))
//通道CHX是否范围溢出，CHX取值TSI_Ch0~TSI_Ch15
#define TSI_IsChxOutRange(CHX)   (TSI0->STATUS&CHX)
//通道CHX是否错误，CHX取值TSI_Ch0~TSI_Ch15
#define TSI_IsChxError(CHX)      (TSI0->STATUS&(CHX<<16))
//使能TSI中断
#define TSI_EnableIrq()     enable_irq(TSI0_IRQn)
//禁用TSI中断
#define TSI_DisableIrq()    disable_irq(TSI0_IRQn)

//TSI通用初始化函数
uint8 TSI_Init(TSI_InitTypeDef);
//TSI反初始化函数
void TSI_Deinit(void);
//编号为ch_num的通道是否有触摸发生
boolean TSI_IsChxTouched(uint8);
