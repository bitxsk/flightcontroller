    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            x = 4;
            SIM->SCGC1 |= SIM_SCGC1_UART4_MASK;///这里不太一样
 ...
 *
 */
char UART_GetChar(UART_Type *uartx)
{
  //等待数据接收
 ...
 *
 */
void UART_PutChar(UART_Type *uartx, char ch)
{
  //等待FIFO准备就绪
 ...
 *
 */
void UART_PutCharArr(UART_Type *uartx, char *ch, int32 len)
{
  while(len--)
  {
...
void SendUNum_3(int32 num){
    uart_putchar(num/1000000);
    uart_putchar(num/10000%100);
    uart_putchar(num/100%100);
}
void SendUNum_2(int32 num){
    uart_putchar(num/100);
    uart_putchar(num%100);
}
void SendUNum_1(int32 num){
    uart_putchar(num);
}

void SendSNum_3(int32 num){
  if(num<0){
    num = -num;
    uart_putchar('-');
  }
