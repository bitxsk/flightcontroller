...
void common_relocate(void)
{
  ...
  */
  ...
  #if defined(__IAR_SYSTEMS_ICC__)
    data_ram = (uint8 *)__section_begin(".data");
    data_rom = (uint8 *)__section_begin(".data_init");
    data_rom_end = (uint8 *)__section_end(".data_init");
  ...
  /* 获得.bss段的地址 (初始化为0的数据) */
  #if defined(__IAR_SYSTEMS_ICC__)
  bss_start = (uint8 *)__section_begin(".bss");
  bss_end = (uint8 *)__section_end(".bss");  
  #else
  #warning 非IAR编译器需确定此处地址
  ...
  */
  #if defined(__IAR_SYSTEMS_ICC__)
  uint8* code_relocate_ram = (uint8 *)__section_begin("CodeRelocateRam");
  uint8* code_relocate = (uint8 *)__section_begin("CodeRelocate");
  uint8* code_relocate_end = (uint8 *)__section_end("CodeRelocate");
