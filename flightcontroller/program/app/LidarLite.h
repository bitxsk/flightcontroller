...
#define kLidarLiteCalculateDistanceMSB          0x8f    // Calculated distance in cm (difference between signal and reference delay)
                                                        // High byte of calculated delay of signal [Read Only]: reference ¨C calculated after correlation record processing
                                                        // If the returned MSB is 1 then the reading is not considered valid.

#define kLidarLiteCalculateDistanceLSB          0x10    // Low byte of calculated delay of signal [Read Only]: reference ¨C calculated after correlation record processing
...
public:
  ...
  COutlierFilter velFilter;
  float vel_z_CF;
  
  void update();
};

extern LidarLite__* LidarLite;
void LidarLiteInit();
#endif // LIDARLITE_H