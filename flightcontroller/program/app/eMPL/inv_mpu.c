...
#include <math.h>
#include "inv_mpu.h"
#include "eMPL/PD_TIME.h"
#include "HW_I2C.h"

I2C_InitTypeDef MPU9150I2C;
...
//		return 0;
//}	  
//#define LPLD_UART_PutCharArr(UART0,...)     do {} while (0)
//#define LPLD_UART_PutCharArr(UART0,...)     do {} while (0)
#define log_e    printf
#define log_i    printf
...
#include "log.h"
#define    i2c_write      msp430_   i2c_write   
#define    i2c_read       msp430_   i2c_read   
...
/*#define i2c_DisableAck(I2Cn)        I2C_C1_REG(I2C0) |= I2C_C1_TXAK_MASK
#define i2c_RepeatedStart(I2Cn)     I2C_C1_REG(I2C0) |= I2C_C1_RSTA_MASK
//启动信号
#define i2c_Start(I2Cn)             I2C_C1_REG(I2C0) |= I2C_C1_TX_MASK;\
                                    I2C_C1_REG(I2C0) |= I2C_C1_MST_MASK
//暂停信号
#define i2c_Stop(I2Cn)              I2C_C1_REG(I2C0) &= ~I2C_C1_MST_MASK;\
                                    I2C_C1_REG(I2C0) &= ~I2C_C1_TX_MASK
//进入接收模式(应答)
#define i2c_EnterRxMode(I2Cn)       I2C_C1_REG(I2C0) &= ~I2C_C1_TX_MASK;\
                                    I2C_C1_REG(I2C0) &= ~I2C_C1_TXAK_MASK
//进入接收模式(不应答)
#define i2c_PutinRxMode(I2Cn)       I2C_C1_REG(I2C0) &= ~I2C_C1_TX_MASK
//等待 I2C0_S
#define i2c_Wait(I2Cn)              while(( I2C_S_REG(I2C0) & I2C_S_IICIF_MASK)==0) {} \
                                    I2C_S_REG(I2C0) |= I2C_S_IICIF_MASK;
//写一个字节
#define i2c_write_byte(I2Cn,data)   I2C_D_REG(I2C0) = data*/

uint8 I2C_ReadByte(uint8 addr, uint8 reg)
{
    uint8 result;
    
    LPLD_I2C_StartTrans( MPU9150I2C.I2C_I2Cx , addr , I2C_MWSR );
    LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
    LPLD_I2C_WriteByte( MPU9150I2C.I2C_I2Cx, reg );
    LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
    LPLD_I2C_ReStart( MPU9150I2C.I2C_I2Cx );
    LPLD_I2C_WriteByte( MPU9150I2C.I2C_I2Cx , (addr<<1)|I2C_MRSW );
    LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
    LPLD_I2C_SetMasterWR( MPU9150I2C.I2C_I2Cx , I2C_MRSW);//转为读 -- I2C_Direc..Receiver
    LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_OFF);//关闭ACK
    result = LPLD_I2C_ReadByte( MPU9150I2C.I2C_I2Cx );
    LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
    LPLD_I2C_Stop( MPU9150I2C.I2C_I2Cx );
    result = LPLD_I2C_ReadByte( MPU9150I2C.I2C_I2Cx );
    //delay
    Pause();
    return result;
...
uint8 i2cWriteBuffer(uint8 addr, uint8 reg, uint8 len, uint8 * data)
{
    ...
    for( i = 0 ; i < len ; i++ ){        
        LPLD_I2C_WriteByte( MPU9150I2C.I2C_I2Cx, data[i] );
        LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
    }
    ...
    if(len == 1){
        ...
        LPLD_I2C_WriteByte( MPU9150I2C.I2C_I2Cx, reg );
        LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
        LPLD_I2C_WriteByte( MPU9150I2C.I2C_I2Cx, *data );
        LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_ON );
        LPLD_I2C_Stop( MPU9150I2C.I2C_I2Cx );
        //delay
    }
...
uint8 i2cRead(uint8 addr, uint8 reg, uint8 len, uint8 *buf)
{
    ...
    while (len) {
        //*buf = I2C_ReceiveByte();
        *buf = LPLD_I2C_ReadByte( MPU9150I2C.I2C_I2Cx );
        if (len == 1)
            //I2C_NoAck();
            LPLD_I2C_WaitAck( MPU9150I2C.I2C_I2Cx , I2C_ACK_OFF );//关闭ACK
        else
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        ...
        if (   i2c_read   (st.hw->addr, ii, 1, &data))
            return -1;
        //LPLD_UART_PutCharArr(UART0,"%#5x: %#5x\r\n", ii, data);
    }
    return 0;
}
...
int mpu_init(void)
{
    ...
    /* 硬件I2C */
    MPU9150I2C.I2C_I2Cx         = I2C1;
    MPU9150I2C.I2C_IntEnable    = FALSE;
    MPU9150I2C.I2C_ICR          = MPU6050_SCL_400KHZ;     //可根据实际电路更改SCL频率
    MPU9150I2C.I2C_SclPin       = PTE1;      //在MPU6050.h中修改该值
    MPU9150I2C.I2C_SdaPin       = PTE0;      //在MPU6050.h中修改该值
    MPU9150I2C.I2C_Isr = NULL;
    LPLD_I2C_Init( MPU9150I2C );
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            LPLD_UART_PutCharArr(UART0,"Unsupported software product rev.\n",34);
            return -1;
        }
    } else {
        ...
        rev = data[0] & 0x0F;
        if (!rev) {
            LPLD_UART_PutCharArr(UART0,"Product ID read as 0 indicates device is either ",48);
            return -1;
        } else if (rev == 4) {
            LPLD_UART_PutCharArr(UART0,"Half sensitivity part found.\n",29);
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (fifo_count < packet_size)
        return 0;
//    LPLD_UART_PutCharArr(UART0,"FIFO count: %hd\n", fifo_count);
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        ...
        if (st_shift[jj]) {
            st_shift_var = st_shift_cust / st_shift[jj] - 1;                 
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    for (jj = 0; jj < 3; jj++) {
        st_shift_cust = labs(bias_regular[jj] - bias_st[jj]) / 65536.f;          //65536.f
        if (tmp[jj]) {
            st_shift = 3275.f / test.gyro_sens;             //3275.f
            while (--tmp[jj])
                st_shift *= 1.046f;                              //1.046f
            st_shift_var = st_shift_cust / st_shift - 1.f;     //1.f
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    unsigned char accel_result, gyro_result;           //unsigned char
#ifdef AK89xx_SECONDARY
    ...
    accel_result = accel_self_test(accel, accel_st);  //这两个函数的问题
    gyro_result = gyro_self_test(gyro, gyro_st);    //还有这个
...
//        LPLD_UART_PutCharArr(UART0,"Compass not found.\n");
//        return -1;
//    }
//
