...
*  备    注：
*************************************************************************/
uint8 I2C_WriteAddr(I2Cn i2cn, uint8 SlaveID, uint8 Addr, uint8 Data)
{
    uint8 result;
    /* send data to slave */
...
uint8 i2cWriteBuffer(uint8 addr, uint8 reg, uint8 len, uint8 * data)//DMP使用的I2C
{
    char i;
    uint8 result;
    /* send data to slave */
    I2C_StartTransmission(0, addr, MWSR);    //启动传输
    i2c_Wait(0);

    i2c_write_byte(0, reg);                    //写地址
    i2c_Wait(0);
for (i = 0; i < len; i++) 
{
    i2c_write_byte(0, data[i]);                    //写数据
    i2c_Wait(0);
}
    i2c_Stop(0);
...
char i2c_write(uint8 addr, uint8 reg, uint8 len, uint8 * data)//DMP使用的I2C
{
...
uint8 i2cRead(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
    uint8 i = 0;
    while (len) 
    {
        buf[i] = I2C_ReadAddr( 0 , addr , reg );
        buf++;
        len--;
    }
