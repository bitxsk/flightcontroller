...
{										    
	...
	unsigned int	wramcmd;		//开始写gram指令
	unsigned int  setxcmd;		//设置x坐标指令
	unsigned int  setycmd;		//设置y坐标指令	 
}_lcd_dev;
...
#define LCD_BASE        ((unsigned long)(0x6C000000 | 0x000007FE))
...
void LCD_SetCursor(unsigned int Xpos, unsigned int Ypos);										//设置光标
void LCD_DrawPoint(unsigned int x,unsigned int y);											//画点
void LCD_Fast_DrawPoint(unsigned int x,unsigned int y,unsigned int color);								//快速画点
unsigned int  LCD_ReadPoint(unsigned int x,unsigned int y); 											//读点 
void LCD_Draw_Circle(unsigned int x0,unsigned int y0,unsigned char r);									//画圆
void LCD_DrawLine(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2);							//画线
void LCD_DrawRectangle(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2);		   				//画矩形
void LCD_Fill(unsigned int sx,unsigned int sy,unsigned int ex,unsigned int ey,unsigned int color);		   				//填充单色
void LCD_Color_Fill(unsigned int sx,unsigned int sy,unsigned int ex,unsigned int ey,unsigned int *color);				//填充指定颜色
void LCD_ShowChar(unsigned int x,unsigned int y,unsigned char num,unsigned char size,unsigned char mode);						//显示一个字符
void LCD_ShowNum(unsigned int x,unsigned int y,unsigned long num,unsigned char len,unsigned char size);  						//显示一个数字
void LCD_ShowxNum(unsigned int x,unsigned int y,unsigned long num,unsigned char len,unsigned char size,unsigned char mode);				//显示 数字
void LCD_ShowString(unsigned int x,unsigned int y,unsigned int width,unsigned int height,unsigned char size,unsigned char *p);		//显示一个字符串,12/16字体
...
void LCD_Set_Window(unsigned int sx,unsigned int sy,unsigned int width,unsigned int height);//设置窗口					   																			 
//9320/9325 LCD寄存器  
#define R0             0x00
#define R1             0x01
#define R2             0x02
