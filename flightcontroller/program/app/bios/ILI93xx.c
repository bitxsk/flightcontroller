...
//写寄存器函数
//regval:寄存器值
void LCD_WR_REG(unsigned int regval)
{ 
...
{   
	...
	unsigned int hsaval,heaval,vsaval,veaval; 
	width=sx+width-1;
	height=sy+height-1;
...
void LCD_Init(void){
    ...
    */
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		LPLD_LCD_Delayms(200); // Dis-charge capacitor power voltage
		LCD_WriteReg(0x0010, 0x1690); // SAP, BT[3:0], AP, DSTB, SLP, STB
		LCD_WriteReg(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		LPLD_LCD_Delayms(50); // Delay 50ms
...
void LCD_Clear(unsigned int color)
{
	...
	unsigned long totalpoint=lcddev.width;
...
//(x,y):中心点
//r    :半径
void LCD_Draw_Circle(unsigned int x0,unsigned int y0,unsigned char r)
{
	int a,b;
	int di;
	a=0;b=r;	  
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(unsigned int x,unsigned int y,unsigned long num,unsigned char len,unsigned char size)
{         	
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(unsigned int x,unsigned int y,unsigned long num,unsigned char len,unsigned char size,unsigned char mode)
{  
	unsigned char t,temp;
	unsigned char enshow=0;						   
	for(t=0;t<len;t++)
