...
{ 
  //调整源地址
  DMA0->TCD[DMA_CH7].SADDR = DMA_SADDR_SADDR(NvidiaTX1->TX.data);
  
  //设置主循环计数器 current major loop count
  DMA0->TCD[DMA_CH7].CITER_ELINKNO = DMA_CITER_ELINKNO_CITER(NvidiaTX1->TX.index);
  
  //起始循环计数器，当主循环计数器为零的时候，将装载起始循环计数器的值
  DMA0->TCD[DMA_CH7].BITER_ELINKNO = DMA_CITER_ELINKNO_CITER(NvidiaTX1->TX.index);
